...
__global__ 
void device_reduce_block_atomic_kernel(int *in, int* out, int N) {
    int sum = int(0);
...
__global__ 
void device_reduce_block_atomic_kernel_vector2(int *in, int* out, int N) {
    int sum=0;
...
void device_reduce_block_atomic_kernel_vector2_range_loop(int *in, int* out, int N) {
    int sum=0;
    for (auto i : grid_stride_range(0, N/2)) {
        int2 val = reinterpret_cast<int2*>(in)[i];
        sum += val.x + val.y;
    }
    int i = blockIdx.x * blockDim.x + threadIdx.x + N/2*2;
    if (i < N) 
        sum += in[i];
    sum = blockReduceSum(sum);
    if (threadIdx.x == 0)
